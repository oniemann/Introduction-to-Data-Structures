...
class jcaltest{
    public static void main(String[] args) {
        ...
        //Non-international route
            ...
            //prints out the month/year calendar
                ...
                find_errors(args, 2);
                misclib.die("illegal day value: use 1-31");
            }
        }
    }
    ...
    static void find_errors(String[] args, int errorno) {
        switch (errorno) {
            case 0:
                ...
                //checks to see that 0 < year < 9999 and presents an
                ...
                break;
            case 1:
                int calmonth = Integer.parseInt (args[0]);
                if(calmonth < 1 || calmonth > 11)
                    misclib.die("illegal month value: use 1-12");
                break;
            case 2:
                try {
                    int calday = Integer.parseInt (args[0]);
                }catch (NumberFormatException error) {
                    String argument = args[0];
                    misclib.die("illegal day value: use 1-31");
                }
    ...
    static void print_week(int[][][] currentYear, 
                                int month, int week, int iterations) {
        for(int day = 0; day < DAYS_IN_WEEK; day++) {
                if(week == 0 && currentYear[month][week][day] == 0)
                    out.printf(day == 0 ? "  " : "   ");
                else {
                    end_of_month(currentYear, month, 
                        week, day, iterations);
                }
        }
        if(iterations == 1)
    ...
    static void print_month(int[][][] currentYear, 
                    int month, int year, Locale locale, int iterations){
    ...
    static void print_year(int[][][] currentYear, 
                    int month, int year, Locale locale, int iterations){
        fit_year(year);
        int thismonth = month;
        ...
        for(int group = 0; group < MONTHS_IN_YEAR / 3; group++) {
            ...
            for(int weekno = 0; weekno < WEEKS_IN_MONTH;  weekno++){
                ...
                while(month < MONTHS_IN_YEAR / 4) {
                    print_week(currentYear, thismonth, 
                        weekno, iterations);
                ...
                //reverts back two months if all the weeks haven't 
                //been looped
    ...
    static void fit_title(int month, int year, 
        int iterations, Locale locale) {
        int sum, bSpaces;
        int extraSpace = 0;
    ...
    static void end_of_month(int[][][] eom, int month, int week, 
        int day, int iterations) {
